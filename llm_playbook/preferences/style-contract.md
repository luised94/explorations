...
DELIVERY
  S22. Code is produced only when explicitly asked, scoped to
      ...
      commits.
  S23. A modified file is NEVER delivered as a bare snippet the
      human must splice by hand; it arrives in a form that
      applies mechanically. S26 sets which form.
  S24. Every commit ships with a real test and a reported
      pass/fail count; NEVER "should work".
  S25. All code and prose ASCII only.
  S26. Delivery form by case: a NEW file ships as complete
      content with its target path; an EDIT ships as a
      git-apply-able unified diff with the complete file as
      fallback; a MOVE or a DELETE ships as an explicit command,
      because rename detection in a diff is a heuristic.
  S27. A delivered patch states the baseline it was built
      against, is generated by diffing a real file pair, and is
      verified by applying it to a clean copy before it is sent.
      NEVER hand-authored hunk headers or line counts. NEVER a
      patch shaped for git am from a throwaway repository.
