...
LENSES (axis -- the question it forces):
  ...
  5. STRUCTURE / INTENT -- does this flatten or sever a meaningful link (e.g.
                           ...
                           vs "this inherits".
  6. DOMAIN CORRECTNESS -- (project-specific; the project's own domain framing,
                           stated in its PROJECT.md) does the atom you are
                           defining stay unambiguous for the features that will
                           build on it?
...
PROJECT GUARDRAILS
--------------------------------------------------------------------------------
Read the project's own guardrails from its PROJECT.md instance rules and the
rendered CONTEXT.md; they are NOT restated here, because a prompt that carries
one project's guardrails will silently apply them to another. Typical shapes,
as a checklist of what to go looking for:
  - data-safety invariants (migration direction, whether the datastore is the
    user's only copy, what a destructive operation is allowed to do)
  - dependency policy (what may be added to the runtime, and under which ADR)
  - the project's standing stance on cleverness and abstraction
  - dispatch or extension points that must be FED rather than forked
