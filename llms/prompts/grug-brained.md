The Grug Brained DeveloperA layman's guide to thinking like the self-aware smol brained
...
THINKÿthey are big brained developers many, many more, and more even definitely probably maybe not like this, many sour face (such is internet)
...
so grug say again and say often: complexityÿvery,ÿveryÿbad
...
grug say prototype early in software making,ÿespeciallyÿif many big brains
...
unit testsÿfine, ok, but break as implementation change (much compared api!) and make refactor hard and, frankly, many bugs anyway often due interactions other code. often throw away when code change.

grug write unit test mostly at start of project, help get things going but not get too attached or expect value long time

end to endÿtests good, show whole system work, but! hard to understand when break and drive grug crazy very often, sometimes grugs just end up ignoring because "oh, that break all time" very bad!

in-between tests, grug hear shaman callÿ"integration tests"ÿsometime often with sour look on face. but grug say integration test sweet spot according to grug: high level enough test correctness of system, low level enough, with good debugger, easy to see what break
...
also, grug dislikeÿmockingÿin test, prefer only when absolute necessary to (rare/never) and coarse grain mocking (cut points/systems) only at that

one exception "first test" dislike by grug: when bug found. grug always try first reproduce bug with regression testÿthenÿfix bug, this case only for some reason work better
...
grug sayÿno silver clubÿfix all software problems no matter what agile shaman say (danger!)
...
also grug notice that introducing too much abstraction often lead to refactor failure and system failure. good example wasÿJ2EEÿintroduce, many big brain sit around thinking too much abstraction, nothing good came of it many project hurt

another good example when company grug work for introduceÿOSGiÿto help manage/trap spriit complexity demon in code base. not only OSGi not help, but make complexity demon much more powerful! took multiple man year of best developers to rework as well to boot! more complex spirit and now features impossible implement! very bad!

Chesterton's Fence

wise grug shamanÿchestertonÿonce say
...
grug once like to minimize lines of code much as possible. write code like this:
 ...
 ÿif(contact && !contact.isActive() && (contact.inGroup(FAMILY) || contact.inGroup(FRIENDS))) {
 ÿ ÿ// ...
 ÿ}
...
over time grug learn this hard debug, learn prefer write like so:
 ...
 ÿif(contact) {
 ÿ ÿvar contactIsInactive = !contact.isActive();
 ÿ ÿvar contactIsFamilyOrFriends = contact.inGroup(FAMILY) || contact.inGroup(FRIENDS);
 ÿ ÿif(contactIsInactive && contactIsFamilyOrFriends) {
 ÿ ÿ ÿ ÿ// ...
 ÿ ÿ}
 ÿ}
...
DRYÿmean Don't Repeat Self, powerful maxim over mind of most developers
...
Separation of Concern (SoC)ÿanother powerful idea over many developer mind, idea to separate different aspects of system into distinct sections code

canonical example from web development: separation of style (css file), markup (html file) and logic (javascript file)

here grug much more sour faced than DRY and in fact write big brained essay on alternative design principleÿlocality of behavior (LoB)ÿagainst SoC
...
funny story: grug learn idolÿrob pikeÿworking on logging at google and decide: "if rob pike working on logging, what grug do there?!?" so not pursue. turn out loggingÿveryÿimportant to google so of course best programmer work on it, grug!

don't be such grug brain, grug, much less shiney rock now!

oh well, grug end up at good company anyway and rob pike dress habitÿincreasingly erratic, so all work out in end, but point stand: logging very important!
...
unfortunately log libraries often very complex (java,ÿwhy you do?) but worth investing time in getting logging infrastructure "just right" pay off big later in grug experience
...
optimistic concurrencyÿseem work well for web stuff

occasionally grug reach forÿthread local variable, usually when writing framework code

some language have good concurrent data structure, like javaÿConcurrentHashMapÿbut still need careful grug work to get right

grug has never usedÿerlang, hear good things, but language look wierd to grug sorry
...
usually grug not care too deeply about detail of api: want write file or sort list or whatever, just want to callÿwrite()ÿorÿsort()ÿor whatever

but big brain api developers say:

"not so fast, grug! is that fileÿopen for write? did you define aÿComparatorÿfor that sort?"
...
grug recognize that big brain api designer have point and thatÿsometimeÿthese things matter, but often do not. big brain api developers better if design for simple cases with simple api, make complex cases possible with more complex api
...
put common thing likeÿfilter()ÿon list and make return list, listen well big brain java api developer!

nobody care about "stream" or even hear of "stream" before, is not networking api, all java grugs use list mr big brain!

Parsing

grug love make programming language at drop of hat and sayÿrecursive descentÿmost fun and beautiful way create parser
...
grug very elated find big brain developer Bob Nystrom redeem the big brain tribe and write excellent book on recursive descent:ÿCrafting Interpreters

book available online free, but grug highly recommend all interested grugs purchase book on general principle, provide much big brain advice and grug love bookÿveryÿmuch except visitor pattern (trap!)
...
grug makeÿhtmxÿandÿhyperscriptÿto avoid
...
note: important to make thinking face and look big brained when saying though. be prepare for big brain or, worse and much more common,ÿthinksÿis big brain to make snide remark of grug
...
now, grug make softwares of much work andÿmoderate open source successÿ, and yet grug himself often feel not any idea what doing! very often! grug still fear make mistake break everyone code and disappoint other grugs, imposter!
...
youÿsay: complexityÿvery,ÿveryÿbad
