...
echo "  Marker totals:"
for m in "Intent:" "Done:" "Noticed:" "Q:" "Connection:"; do
  printf "    %-14s %d\n" "$m" "$(grep -c "$m" "$J" 2>/dev/null || echo 0)"
done
echo
echo "  Per-date marker presence:"
awk '/^## [0-9]{4}-/{date=$2}
     /Intent:/{i[date]++} /Done:/{d[date]++}
     /Noticed:/{n[date]++} /Q:/{q[date]++}
     END{for(dt in i)
       printf "    %s  Intent:%d  Done:%d  Noticed:%d  Q:%d\n",
...
printf "  Note sections:        %d\n" "$(grep -c '^## @' "$N")"
printf "  Forward links:        %d\n" "$(grep -c '^-> @' "$N" 2>/dev/null || echo 0)"
printf "  Block content lines:  %d\n" "$(grep -c '^\s*>' "$N" 2>/dev/null || echo 0)"
...
# Intent completion
intent_c=$(grep -c 'Intent:' "$J" 2>/dev/null || echo 0)
done_c=$(grep -c 'Done:' "$J" 2>/dev/null || echo 0)
if (( intent_c > 0 )); then
...
q_c=$(grep -c 'Q:' "$J" 2>/dev/null || echo 0)
printf "  Open questions captured: %d" "$q_c"
(( q_c == 0 )) && echo "  (none -- drop Q: or start using it)" || echo ""

conn_c=$(grep -c 'Connection:' "$J" 2>/dev/null || echo 0)
...
echo "Audit complete -> $OUT"!/usr/bin/env bash
# kbd-audit.sh -- consolidated metrics for kbd plain-text note system
# Run from the root of your kbd repo.
# Usage: bash kbd-audit.sh [output_file]
