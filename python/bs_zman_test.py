...
print(
    str(soup.find_all("article")[0].find("time", class_="entry-date").string)
    .replace(" ", "_")
    .replace(",", "")
)
...
for date in soup.find(id="archives-2").ul.find_all("a"):
    ...
    for article in soup_date.find_all("article"):
        f.write(
            str(article.find("h1", class_="entry-title").string)
            + "_"
            + str(article.find("time", class_="datetime").string)
            .replace(" ", "_")
            .replace(",", "")
            + "\n"
        )
