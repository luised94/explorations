...
service = Service(
    "https://yeastmine.yeastgenome.org/yeastmine/service", username="", password=""
)
        ...
        for row in query.rows():
            ...
            for view in views:
                string_holder = str(
                    ...
                    ",", "_"
                )  # string holder to be able to modify twice, replace , with _ to write to csv
                if count == len(views_list):
                    row_string += (
                        string_holder.replace("\n", "") + "\n"
...
def query_to_fasta(
    ...
    listTypes,
    type_of_ARS_List,
    view_for_fasta_heading,
    short_description,
    view_for_fasta_content,
):
    today = str(date.today())
    ...
    for row in query.rows():
        ...
        index_to_add_new_line = [
            x for x in range(len(sequence_to_write) + 1) if x % 40 == 0
        ]
...
df = pd.read_excel(
    r"C:\Users\liusm\Desktop\ARS and Gene Analysis\ARS_transcription_overlap.xlsx"
)
...
for i in range(0, 6):
    lm.create_list(
        content=df["ARS_List"][i],
        list_type="ARS",
        name="ARS_List_" + type_of_ARS_list[i],
    )
    # Create list for each ARS type for the respective gene in the list
for i in range(3, 5):
    for j in range(0, 6):
        lm.create_list(
            content=df[header_list[i]][j],
            list_type="Gene",
            name=header_list[i] + "_" + type_of_ARS_list[j],
        )
...
for i in range(1, 3):
    for j in range(0, 6):
        ...
        query.add_constraint(
            "Gene", "IN", list_type[i] + "_" + type_of_ARS_list[j], code="A"
        )
